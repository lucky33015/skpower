                    ...
                    <div slot="left" class="demo-split-pane">
                        <template>
                            <i-form ref="formValidate" :model="formValidate" :rules="ruleValidate" label-position="top">
                                <form-item label="发送语音" prop="media_id">
                                    <upload
                                        ref="upload"
                                        type="drag"
                                ...
                                <form-item>
                                    ...
                                    <i-button type="primary" to="{{url('skpower/test/down',['voice'])}}" style="margin-left: 8px">点击下载测试文件
                                    </i-button>
    ...
    <script>
        new Vue({
            ...
            data: {
                ...
                ruleValidate: {
                    media_id: [
                        {required: true, message: '没有获取到语音文件的media_id,请检查后重新上传!', trigger: 'blur'}
                    ],
                    user_id: [
            ...
            methods: {
                ...
                handleFormatError (file) {
                    this.$Message.error(file.name + ' 文件格式不正确,仅支持amr格式音频.');
                },
                handleMaxSize (file) {
                    this.$Message.error(file.name + ' 超出文件大小限制,最大上传2M,播放长度不超过60s');
                },
                handleSubmit(name) {
                    this.$refs[name].validate((valid) => {
                        if (valid) {
                            var url = "{{url('skpower/voice/send')}}";
                            this.$Loading.start();
                            this.$http.post(url, {
                                voice: this.formValidate.media_id,
