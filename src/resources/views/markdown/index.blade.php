                    ...
                    <div slot="left" class="demo-split-pane">
                        <template>
                            <i-form ref="formValidate" :model="formValidate" :rules="ruleValidate"  label-position="top" >
                                ...
                                <form-item label="markdown内容:" prop="description">
                                    <i-input v-model="formValidate.description" placeholder="请输入markdown格式文本内容"
                                             maxlength="1000" show-word-limit type="textarea" :autosize="true"
                                    />
                                </form-item>
    ...
    <script>
        new Vue({
            ...
            data: {
                ...
                ruleValidate: {
                    description: [
                        {required: true, message: '请填写发送markdown!', trigger: 'blur'}
                    ],
                    user_id: [
            ...
            methods: {
                handleSubmit(name) {
                    this.$refs[name].validate((valid) => {
                        if (valid) {
                            var url = "{{url('skpower/markdown/send')}}";
                            this.$Loading.start();
                            this.$http.post(url, {
