                    ...
                    <div slot="left" class="demo-split-pane">
                        <template>
                            <i-form ref="formValidate" :model="formValidate" :rules="ruleValidate" label-position="top">
                                <form-item label="发送文件" prop="media_id">
                                    <upload
                                        ref="upload"
                                        type="drag"
                                    ...
                                    <span style="color: green;">*-文件发送支持的格式很多,目前只尝试了这几种,所以,上传文件的格式此处不做任何限制['docx','txt','xlsx','pptx','xmind','mp4','amr','jpg','png','md']-*</span>
                                </form-item>
                                ...
                                <form-item>
                                    ...
                                    <i-button type="primary" to="{{url('skpower/test/down',['file'])}}" style="margin-left: 8px">点击下载测试文件
                                    </i-button>
    ...
    <script>
        new Vue({
            ...
            data: {
                ...
                ruleValidate: {
                    media_id: [
                        {required: true, message: '没有获取到文件的media_id,请检查后重新上传!', trigger: 'blur'}
                    ],
                    user_id: [
            ...
            methods: {
                ...
                handleMaxSize (file) {
                    this.$Message.error(file.name + ' 超出文件大小限制,最大上传20M');
                },
                handleSubmit(name) {
                    this.$refs[name].validate((valid) => {
                        if (valid) {
                            var url = "{{url('skpower/file/send')}}";
                            this.$Loading.start();
                            this.$http.post(url, {
                                file: this.formValidate.media_id,
